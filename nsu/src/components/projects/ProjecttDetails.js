...
const ProjecttDetails = (props) => {
  const { comments, projectt, auth } = props;
            ...
            <div className="card z-depth-0 details">
              <div className="card-content">
                <span className="card-title">
                  <span className="blue-text">{projectt.subs}</span>
                </span>
                <p>
                  <b style={{ fontWeight: "bold" }}>From : </b>
                  <span className="green-text">{projectt.salary}</span>
                </p>
                <p>
                ...
                <p className="center">{projectt.description}</p>
              </div>
              <div className="card-action">
                <div className="overflows">
                  <img className="detailImage" src={projectt.image} height="700" width="350" alt=" " />
                </div>
              </div>
              <br></br>
...
const mapStateToProps = (state, ownProps) => {
  ...
  const projectts = state.firestore.data.projectts;
  const projectt = projectts ? projectts[id] : null;
  return {
    projectt: projectt,
